                    ...
                    <div data-flex-no-shrink>
                        <div class="nav-fold dropup">
                            <a data-toggle="dropdown">
                                <?php 
                               $sql_user = "SELECT userid, ImageFile from tb_user where userID = '$userid'";
                               $query_user = mysqli_query($link, $sql_user);
                               $r_user = mysqli_fetch_all($query_user);
                               $result = $r_user[0]
                                ?>
                                <span class="pull-left">
                                    <img src="database/images/User/<?php echo $result[1]?>" alt="..." class="w-32 img-circle">
                                </span>
                                <span class="clear hidden-folded p-x p-y-xs">
                                    <span class="block _500 text-ellipsis"><?php echo $result[0]?></span>
                                </span>
                            </a>
                                ...
                                <div class="padding">
                                    ...
                                    <div class="page-title m-b">
                                        ...
                                        <div class="dropdown inline">
                                            <button class="btn btn-sm no-bg h4 m-y-0 v-b dropdown-toggle text-primary" data-toggle="dropdown">All the time</button>
                                            <div class="dropdown-menu">
                                                <a href="chart.php" class="dropdown-item active">
                                                    All the time
                                                </a>
                                                <a href="chartLastMonth.php" class="dropdown-item">
                                                    Last month
                                                </a>
                                                <a href="chartLastYear.php" class="dropdown-item">
                                                    Last year
                                                </a>
                                                <a href="chartLastWeek.php" class="dropdown-item">
                                                    Last week
                                                </a>
                                            </div>
                                        </div>
                                    ...
                                    <?php
                                    $sql = "SELECT TrackName, sum(TotalListen), trackfile, ImageFile, TrackID, ArtistName, TotalLiked, albumID, ArtistID FROM vw_track_listen_like_info group by TrackID ORDER BY SUM(TotalListen) DESC ,TrackName ASC LIMIT 10";
                                    $r = mysqli_query($link, $sql);
                                    $char = mysqli_fetch_all($r);
                                    foreach ($char as $key_char => $songs) {
                                    $num_songs = $key_char + 1;
                                        ?>

                                    <div class="row item-list item-list-md item-list-li m-b">
                                            <div class="col-xs-12">
                                                <div class="item r" data-id="item-<?php echo $num_songs + 12 ?>" data-src="<?php echo $songs[2] ?>">
                                                    <div class="item-media ">
                                                        <a href="album.php?albumid=<?php echo $songs[7]?>" class="item-media-content" style="background-image: url('<?php echo $songs[3] ?>');"></a>
                                                        <div class="item-overlay center">
                                                            <button  class="btn-playpause" onclick="window.location.href= 'listentime_savedata.php?trackid=<?php echo $songs[4] ?>&url=<?php echo getCurURL() ?>'">Play</button>
                                                        </div>
                                                    </div>
                                                    <div class="item-info">
                                                        <div class="item-overlay bottom text-right">
                                                            <!-- Like Button-->
                                                            <form class= "btn-favorite" action="Like_Track_savedata.php" id='like_track_<?php echo $songs[4] ?>' >
                                                                <input style = "display:none" type="text" name="txtaction" <?php if (userLiked($songs[4])): ?> value= "unlike" <?php else : ?>value= "like" <?php endif ?> readonly>
                                                                <input style = "display:none" type="text" name="txttrackid" value="<?php echo $songs[4] ?>" readonly>
                                                                ...
                                                                <input style = "display:none" type="text" name="txturlid" value="like_track_<?php echo $songs[4] ?>" readonly>           

                                                                <button type="submit " name="btnLike" style="background:transparent;border:none;"
                                                                <?php if (userLiked($songs[4])): ?> 
                                                                            class="likes fa fa-heart like-btn" 
                                                                        <?php else: ?> 
                                                                            class="likes fa fa-heart-o like-btn" 
                                                                        <?php endif ?> 
                                                                        data-id="<?php echo $songs[4] ?>"> 
                                                                </button>

                                                                <!--  Like Counter-->
    <!--                                                                            <span class="likes counter"><?php echo getLikes($songs[4]); ?> </span>-->
                                                                <!-- Button MORE -->
                                                                <a href="#" class="btn-more" data-toggle="dropdown"><i class="fa fa-ellipsis-h"></i></a>
                                                                <div class="dropdown-menu pull-right black lt">
                                                                    <a class="dropdown-item" href="add_to_queue_savedata.php?trackid=<?php echo $songs[4] ?>&url=<?php echo getCurURL() ?>"><i class="fa fa-plus fa-fw text-left"></i> Add to Queque</a>
                                                                        ...
                                                                        <div class="dropdown-menu pull-left dark" role="menu">
                                                                            ...
                                                                            if ($total_list_playlist > 0):
                                                                                ...
                                                                                foreach ($a_list_playlist as $list_playlist):
                                                                                    ?>
                                                                                    <a class="dropdown-item" href="add_to_playlist_savedata.php?playlistid=<?php echo $list_playlist[0] ?>&url=<?php echo getCurURL() ?>&trackid=<?php echo $songs[4] ?>">Add to <?php echo $list_playlist[1] ?></a>
                                                        ...
                                                        <div class="item-title text-ellipsis">
                                                            <a href="album.php?albumid=<?php echo $songs[7] ?>"><?php echo $songs[0] ?></a>
                                                        </div>
                                                        <div class="item-author text-sm text-ellipsis ">
                                                            <a href="artist.detail.php?artistid=<?php echo $songs [8]?>" class="text-muted"><?php echo $songs[5]?></a>
                                                        </div>
                                                        <div class="item-meta text-sm text-muted">
                                                            <span class="item-meta-stats text-xs  item-meta-right">
                                                                <i class="fa fa-play text-muted"></i> <?php echo $songs[1] ?>
                                                                <i class="fa fa-heart m-l-sm text-muted"></i><?php echo $songs[6]?>
                                                            </span>
                                                        </div>
